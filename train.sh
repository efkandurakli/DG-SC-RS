...
python train.py  --output-dir output/RGB+NIR+SWIR_LB --band-groups rgb nir swir --train-folders train test-10% --use-deterministic-algorithms --batch-size 128 --patience 200 --max-epochs 200  --lr 0.0002 # upper bound
python train_dg.py  --output-dir output/RGB+NIR+SWIR_LB --band-groups rgb nir swir --train-folders train --use-deterministic-algorithms --batch-size 128 --patience 200 --max-epochs 200  --lr 0.0002 # domain generalization
