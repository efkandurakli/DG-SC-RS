...
from mixstyle import resnet18
from constants import *

def train_one_epoch(model, train_iter, criterion, optimizer, device, epoch, args):
    model.train()
    ...
    for i in range(args.iters_per_epoch):
        ...
        cls_acc = utils.accuracy(y, labels)[0]
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.max_epochs):
        train_loss = train_one_epoch(model, train_iter, criterion_class, optimizer, device, epoch, args)
        lr_scheduler.step()
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument('--n-domains-per-batch', default=2, type=int, help='number of domains in each mini-batch')
