...
class BigEarthNetDataset(Dataset):
    def __init__(self, root, phase="train", transform=None):
        ...
        self.img_labels = os.listdir(os.path.join(root, phase, "labels"))
        
    def __len__(self):
        return len(self.img_labels)

    def __getitem__(self, idx):
        json_path = os.path.join(self.root, self.phase, "labels", self.img_labels[idx])
        ...
        image_files = os.listdir(os.path.join(self.root, self.phase, "images", images_folder))
        ...
        band_b2_ds = gdal.Open(os.path.join(self.root, self.phase, "images", images_folder, b2_file),  gdal.GA_ReadOnly)
        raster_band_b2 = band_b2_ds.GetRasterBand(1)
        band_data_b2 = raster_band_b2.ReadAsArray()
        
        band_b3_ds = gdal.Open(os.path.join(self.root, self.phase, "images", images_folder, b3_file),  gdal.GA_ReadOnly)
        raster_band_b3 = band_b3_ds.GetRasterBand(1)
        band_data_b3 = raster_band_b3.ReadAsArray()
        
        band_b4_ds = gdal.Open(os.path.join(self.root, self.phase, "images", images_folder, b4_file),  gdal.GA_ReadOnly)
...
normalize = transforms.Normalize(mean=[429.9430203, 614.21682446, 590.23569706],
...
dataset = BigEarthNetDataset("data/bigarthnet", transform=transform)

dataloader = DataLoader(dataset, batch_size=1, shuffle=True)

for image, label, domain in dataloader:
